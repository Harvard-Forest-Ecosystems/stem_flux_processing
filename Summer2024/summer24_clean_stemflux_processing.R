...
file_sources <- list.files("naomi/Summer2024/functions", pattern="*.R", full.names = TRUE)
...
date_time_raw <- read_csv("data/summer_2024.csv") %>%
...
init$outfilename <- "summer2024_clean.csv"
init$pdffilename <- "summer2024check1.pdf"
...
#-----------------------------------#
#-----------------------------------#
# Load in updated times spreadsheet
time_updates <- read_csv('timing_updates.csv') %>%
  rename('UniqueID' = `Unique ID`)

# combine with date time df and update time column
date_time_up <- left_join(date_time_met, time_updates, by = 'UniqueID')

# rename columns
date_time_up <- date_time_up %>%
  rename('old_start' = 'start_time',
         'old_end' = 'end_time')%>%
  mutate(start_time = lubridate::ymd_hms(paste0(dates,Updated_start)),
         end_time = lubridate::ymd_hms(paste0(dates,Updated_end)))%>%
  filter(quality == 'good')
...
summer24 <- calculate_chamber_flux(conc_data, date_time_up, init)   
...
#-----------------------------------#
# read in dataframe
flux <- read_csv("output/summer2024_clean.csv")

# subset raw data with important columns
date_time_important <- subset(date_time_up, select = c('UniqueID',
                                                       'real_start',
                                                       'Tree'))
...
# export
write_csv(flux_full,"output/summer2024_clean.csv")
