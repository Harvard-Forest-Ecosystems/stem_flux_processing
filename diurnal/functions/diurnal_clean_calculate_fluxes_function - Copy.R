...
calculate_diurnal_flux <- function(conc_data, date_time, init){
  ...
  flux_data <- data.frame(date = empty_vector,
                          height = empty_vector,
  ...
  for(c in 1:nrow(date_time)){
    ...
    flux_data$date[rep_index]  <- date_time$`Start (iPad)`[c]
    flux_data$height[rep_index]  <- date_time$`Height (m)`[c]
