...
file_sources <- list.files("naomi/fall2024_onwards/functions", pattern="*.R", full.names = TRUE)
...
#-----------------------------------#
#-----------------------------------#
# Load all files from data folder with sampling dates and start times for flux measurements
# and stitch together

# Point r to folder with files
filenames <- dir(path = here("insert folder path"), pattern = "Stem*", full.names = TRUE)
...
# Once files have been checked, uncomment the following
# %>%
  #filter(check == 'good')
...
init$outfilename <- "insert file name here"
init$pdffilename <- "inset pdf file name here"
